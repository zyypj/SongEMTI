...
export default function PlayerBar() {
  ...
  useEffect(() => {
    if (!player) return;
    if (!player.isMuted?.()) {
      player.setVolume?.(desiredVolRef.current);
    }
  ...
  return (
    <div className="fixed inset-x-0 bottom-0 z-50 border-t border-white/10 bg-neutral-950/80 backdrop-blur supports-[backdrop-filter]:bg-neutral-950/60">
